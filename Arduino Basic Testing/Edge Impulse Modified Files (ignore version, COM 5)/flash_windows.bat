...
REM Check if the CLI version is at least 0.18.x or any 1.x.x or higher
if !CLI_MAJOR! EQU 0 (
    if !CLI_MINOR! LSS !EXPECTED_CLI_MINOR! (
        GOTO UPGRADECLI
    )
) else if !CLI_MAJOR! LSS 0 (
    GOTO UPGRADECLI
)

REM Major version is 1 or higher and is acceptable
if !CLI_MAJOR! GTR 0 (
    echo You're using an acceptable version of Arduino CLI: %CLI_MAJOR%.%CLI_MINOR%.%CLI_REV%
) else (
    if !CLI_MAJOR! EQU !EXPECTED_CLI_MAJOR! (
...
REM Manually set the COM port to COM5
set COM_PORT=COM5

REM Uncomment below block if auto-detecting the COM port
REM set COM_PORT=""
REM for /f "tokens=1" %%i in ('arduino-cli board list ^| findstr "Arduino Nano 33 BLE" ^| findstr "COM"') do (
REM    set COM_PORT=%%i
REM )
